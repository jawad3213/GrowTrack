...
onMounted(async () => {
  try {
    // Total users
    const resUsers = await api.get('/api/DashAdmin/Total_User')
    totalUsers.value = resUsers.data

    // Growth in users
    const resUserGrowth = await api.get('/api/DashAdmin/Stat_User')
    userGrowth.value = resUserGrowth.data.percentage
    trendUser.value = resUserGrowth.data.trend

    // Total evaluations
    const resEvals = await api.get('/api/DashAdmin/Total_Evaluation')
    totalEvaluations.value = resEvals.data

    // Growth in evaluations
    const resEvalGrowth = await api.get('/api/DashAdmin/Stat_Evaluation')
