...
onMounted(async () => {
  try {
    // API 1 : pourcentage
    // const currentRes = await api.get('/api/DashAdmin/Stat_Involvement')
    // totalInvolvement.value = currentRes.data.percentage

    // API 2 : subblitted
    const sub = await api.get(`/student/dashboard/evaluation_submitted/${auth.ID}`)
    growth.value = sub.data.data.total
    console.log(growth.value )
    // AP 3 : assigned
    const assig = await api.get(`/student/dashboard/evaluation_assigned/${auth.ID}`)
