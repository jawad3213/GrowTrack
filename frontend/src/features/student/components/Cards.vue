...
onMounted(async () => {
  try {
    // Projects enrolled
    const resUsers = await api.get(`/student/dashboard/nombre_projets/${auth.ID}`)
    totalUsers.value = resUsers.data.data.count

    // class score
    const resUserGrowth = await api.get(`/student/dashboard/moyenne/${auth.ID}`)
    userGrowth.value = resUserGrowth.data.data.moyenne

    // Total evaluations
    const resEvals = await api.get(`/student/dashboard/signalreceived/${auth.ID}`)
    totalEvaluations.value = resEvals.data.data.total

    // Growth in evaluations
    // const resEvalGrowth = await axios.get('/api/DashAdmin/Stat_Evaluation')
