...
const confirmDelete = async (id_project) => {
  if (confirm('Voulez-vous vraiment supprimer ce projet ?')) {
    try {
      await api.delete(`/api/prof_project_management/delete_project/${id_project}`);
      projects.value = projects.value.filter((p) => p.id_project !== id_project);
    } catch (error) {
