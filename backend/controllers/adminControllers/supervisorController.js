...
exports.getSupervisorById = async (req, res) => {
  ...
  try {
    const supervisor = await supervisorModel.getSupervisorById(id);
    ...
    console.error("Error retrieving supervisor by ID:", error);
    return res.status(500).json({
      error: "Internal server error. Please try again later.",
    });
  }
...
exports.getSupervisorsByPosition = async (req, res) => {
  ...
  exports.getSupervisorStats = async (req, res) => {
    try {
      const stats = await supervisorModel.getSupervisorStats();
      return res.status(200).json({
        message: "Supervisor statistics retrieved successfully.",
        data: stats
      });
    } catch (error) {
      console.error("Error retrieving supervisor statistics:", error);
      return res.status(500).json({
        message: "Internal server error. Please try again later.",
      });
    }
