...
exports.getSkillStats = async (req, res) => {
  try {
    ...
    console.error("Error retrieving skill statistics:", error.message);
    return res.status(500).json({
      message: "Internal server error. Please try again later.",
    });
  }
...
exports.getEvaluationStats = async (req, res) => {
  try {
    const stats = await skillModel.getEvaluationStats();
    return res.status(200).json({
      message: "Evaluation statistics retrieved successfully.",
      data: stats
    });
  } catch (error) {
    console.error("Error retrieving evaluation statistics:", error.message);
    return res.status(500).json({
      message: "Internal server error. Please try again later.",
    });
  }
...
exports.getSignalStats = async (req, res) => {
  try {
    const stats = await skillModel.getSignalStats();
    return res.status(200).json({
      message: "Signal statistics retrieved successfully.",
      data: stats
    });
  } catch (error) {
    console.error("Error retrieving signal statistics:", error.message);
    return res.status(500).json({
      message: "Internal server error. Please try again later.",
    });
  }
