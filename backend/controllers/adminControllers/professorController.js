...
exports.getProfessorById = async (req, res) => {
  const { id } = req.params;
  console.log('🔥 getProfessorById called with id:', id);

  try {
    const professor = await professorModel.getProfessorById(id);
    console.log('🔥 professor from model:', professor);
...
exports.getProfessorsByClass = async (req, res) => {
  ...
  exports.getProfessorsBySector = async (req, res) => {
    ...
    try {
      const professors = await professorModel.getProfessorsBySector(sector);
  ...
  exports.getProfessorStats = async (req, res) => {
    try {
      const stats = await professorModel.getProfessorStats();
      return res.status(200).json({
        message: "Professor statistics retrieved successfully.",
        data: stats
      });
    } catch (error) {
      console.error("Error retrieving professor statistics:", error);
      return res.status(500).json({
        message: "Internal server error. Please try again later.",
      });
    }
