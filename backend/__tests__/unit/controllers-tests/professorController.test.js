  ...
  describe("getProfessorsBySector", () => {
    describe("Happy Path", () => {
      test("should return professors by sector successfully", async () => {
        ...
        professorModel.getProfessorsBySector.mockResolvedValue(mockProfessors);
        req.body = { sector: "Engineering" };

        await professorController.getProfessorsBySector(req, res);

        expect(professorModel.getProfessorsBySector).toHaveBeenCalledWith("Engineering");
        expect(res.status).toHaveBeenCalledWith(200);
        expect(res.json).toHaveBeenCalledWith({
    ...
    describe("Unhappy Path", () => {
      test("should handle database error", async () => {
        professorModel.getProfessorsBySector.mockRejectedValue(new Error("Database error"));
        req.body = { sector: "Engineering" };
