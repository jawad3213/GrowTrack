...
describe('Rate Limiters Configuration', () => {
  ...
  describe('Authlimiter Configuration', () => {
    ...
    it('should not have an onLimitReached callback (uses default rate limit behavior)', () => {
      require('../../../middlewares/Limiter');
      ...
      const authConfig = authCall[0];
      // The Authlimiter uses keyGenerator for logging, but no onLimitReached callback
      expect(authConfig.onLimitReached).toBeUndefined();
    });
