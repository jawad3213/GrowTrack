    ...
    try {
        const result = await pool.query(
            `SELECT 
                ...
                reporter.prenom || ' ' || reporter.nom AS reporter_name,
                reporter.my_role AS reporter_role,
                reported.profile_picture AS reported_picture,
                reported.prenom || ' ' || reported.nom AS reported_name,
                reported.my_role AS reported_role
            FROM public.signal s
            LEFT JOIN public.utilisateur reporter ON reporter.id_utilisateur = s.reporter_id
            LEFT JOIN public.utilisateur reported ON reported.id_utilisateur = s.reported_id
...
exports.solution = async (id_signal, option_solution, details, name_coach, start_date, date_done) => {
  try {
    ...
    const coach = await pool.query(
      `SELECT id_utilisateur FROM public.utilisateur WHERE prenom || ' ' || nom = $1`,
      [name_coach]
    );
    ...
    const signal = await pool.query(
      `SELECT reported_id FROM public.signal WHERE id_signal = $1`,
      [id_signal]
    );
    ...
    await pool.query(
      `INSERT INTO public.follow_up (id_coach, id_student, id_solution, message, start_date, date_done)
       VALUES ($1, $2, $3, $4, $5, $6)`,
      [coach.rows[0].id_utilisateur, signal.rows[0].reported_id, solution.rows[0].id_solution, details, start_date, date_done]
    );
    ...
    await pool.query(
      ...
      [details,signal.rows[0].reported_id]
    )
